	...
	while(1)
	{
		...
		else
		{
			...
			else if(dlugosc1 > 0)
			{
				...
				// mapowanie 
				ftruncate(mapplik, dane_pliku.st_size + dlugosc1);
				wskaznik_do_pamieci = mmap(NULL, dane_pliku.st_size + dlugosc1, PROT_READ | PROT_WRITE, MAP_SHARED, mapplik, 0);
				...
				{
					wskaznik_do_pamieci[dlugosc+i++]=bufor[0];
				}
				printf(" zakonczone :)\n");
			}
			close(plik);
